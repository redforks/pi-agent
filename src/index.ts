...
/**
 * pi-notify: `on_agent_finish` (ticket #8), native `on_ask_user` (ticket
 * #11) and extra-tool `on_ask_user` (ticket #12) paths.
 *
 ...
 * `agent_settled`, `ui_prompt_start` and `tool_call`. On each settle where
 * `ctx.isIdle()` is true, every `on_agent_finish` command runs in array order
 * through `sh -c`, fire-and-forget. On each observed `ui_prompt_start` — one
 * per waiting span, since the host coalesces nested/overlapping prompts into
